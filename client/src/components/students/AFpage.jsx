import React from "react";
import "../App.css";

function AFpage() {
  return (
    <div class="AFstyles">

      <div>
        <br /><h2 class="FirstH1"><pre>  Welcome to Application Frameworks SE3040!</pre></h2><br />
        <p><b><pre>   • LIC: Mr. Thusithanjana Thilakarathna</pre></b>
          <pre>        Email - thusithanjana.t@sliit.lk</pre>
          <b><pre>   • Instructor: Ms. Karthiga Rajendran</pre></b>
          <pre>        Email - karthiga.r@sliit.lk</pre>
          <b><pre>   • Visiting Lecturer: Mr. Kushira Godellawatta</pre></b>
          <pre>        Email - kushira@gmail.com</pre>
        </p>
      </div>

      <div >
        <hr className="AFPageHr1" />
      </div>

      <h3>&nbsp;&nbsp;&nbsp;Project Guidance</h3>
      <a href="https://drive.google.com/file/d/1I7_z4pnlJnfYCKqPoGfzVQg8TjLcne3K/view?usp=sharing">
        <button type="button" className="button8">
          <pre> &nbsp;&nbsp;&nbsp;📋 Project and module otline</pre>
        </button>
      </a><br />
      <a href="https://drive.google.com/file/d/1vFXHj0wD7Pro6COhqgU2-ifa6xFzLmSa/view?usp=sharing">
        <button type="button" className="button8">
          <pre> &nbsp;&nbsp;&nbsp;📋 Project 1 - Pdf</pre>
        </button><br />
      </a>
      <a href="https://drive.google.com/file/d/1FJjWcBGGvr04ylh9Nlknc3qlCRGPlmF4/view?usp=sharing">
        <button type="button" className="button8">
          <pre> &nbsp;&nbsp;&nbsp;📋 Project 2 - Pdf</pre>
        </button>
      </a>


      <div >
        <hr className="AFPageHr2" />
      </div>

      <h3>&nbsp;&nbsp;&nbsp;Download Templates</h3>
      <div className="navItem">
        <a href="https://drive.google.com/drive/folders/1FAqgy82XTmFLWLFAyKY9XbEyghlI94i1?usp=sharing">
          <button type="button" className="Bts10">
            <pre>Download Templates - Click here...</pre>
          </button>
        </a>
      </div>

      <div >
        <hr className="AFPageHr2" />
      </div>





      <div>
        <h3>&nbsp;&nbsp;&nbsp;Submissions</h3>
        <h5>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Submit your project and project matirials below</h5>
      </div>
      <div className="navItem">
        <a href="/StdDocDocument">
          <button type="button" className="AFRightNavBarButtons button1">
            <pre>Student Document</pre>
          </button>
        </a>
      </div>
      <br /><br /><br />



      <div class="AFRightNavBar">
        <div class="AFRightNavBarBody">

          <div className="navItem">
            <a href="/StudentGroup">
              <button type="button" className="AFRightNavBarButtons button1">
                <pre> Student Groups</pre>
              </button>
            </a>
          </div><br />

          <div className="navItem">
            <a href="/ResearchTopic">
              <button type="button" className="AFRightNavBarButtons button1">
                <pre>Research Topic</pre>
              </button>
            </a>
          </div><br />

          <div className="navItem">
            <a href="/ViewMarksNavPage">
              <button type="button" className="AFRightNavBarButtons button1">
                <pre>View Marks</pre>
              </button>
            </a>
          </div><br />

          <div className="navItem">
            <a href="https://chat.whatsapp.com/COcoBXUJo4812jj5QS3Nns">
              <button type="button" className="AFRightNavBarButtons button1">
                <pre>Chat With Supervisor</pre>
              </button>
            </a>
          </div>


        </div>
      </div>

    </div>
  )
}


export default AFpage;

//https://drive.google.com/drive/folders/1-YTswm8zPPp4P7eodwATGxHIiXc5wXuI?usp=sharing